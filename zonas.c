...
void add_zone(zones **Vzones){ //Vzones es el vector donde se guardan las zonas
	int i=contar(0);
	*Vzones=realloc(*Vzones, (i+1)*sizeof(zones)); //redimension del array Vzones
...
int contar(int key){
	...
	if(key==1)rewind(arch); // si key es 1 rebobina el archivo si no, lo cierra 
	else fclose(arch);      //(el key sirve por si la funcion es llamada dentro de una funcion que ya habra el archivo.dat)
	return acc;
}
...
int val_zone(long idzone){
	...
	int n=contar(0);
	for(i=0;i<n;i++){
		if(idzone==Vzones[i].id_zone){ //busca si la zona existe dentro del archivo
			free(Vzones);
			return 1;
		}
	}
